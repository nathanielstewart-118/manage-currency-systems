...
def handle_deposit():
    try:
        ...
        sql = "UPDATE mids SET deposit = deposit+"+deposit+" WHERE mid =" + mid
        mycursor.execute(sql)
        mydb.commit()
        log_write(mid, area, "deposit : " + deposit)

        return { "message": "Deposit updated", "success": True }
    except Exception as e:
        return { "message": str(e), "success": False}
    
print ("Access-Control-Allow-Headers: Origin, Content-Type\r\nAccess-Control-Allow-Origin: *\r\nAccess-Control-Allow-Methods: GET, POST, PATCH, PUT, DELETE, OPTIONS\r\nContent-Type: application/json\r\n")
print(json.JSONEncoder().encode(handle_deposit()))